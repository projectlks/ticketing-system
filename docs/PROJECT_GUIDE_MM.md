...
- Proxy guard: `src/proxy.ts` (Next.js 16 middleware replacement)
...
Allowed roles = `LEVEL_3`, `SUPER_ADMIN`
...
- `POST /api/uploads` : file upload to `/uploads` (Auth required)
- `GET /api/uploads/[filename]` : serve image (Auth required)
- `DELETE /api/uploads/[filename]` : delete file (Auth required)
...
Note:
- Upload endpoints အားလုံးသည် session auth မရှိရင် `401` ပြန်ပေးပါသည်။
...
- `NEXTAUTH_SECRET` (fallback secret key for auth/proxy)
...
## 6. Uploaded images not loading (or 401)
...
5. **Date library standardization (Resolved)**

- Project is now standardized to `dayjs` only.
...
- Auth: `src/app/api/auth/[...nextauth]/route.ts`, `src/proxy.ts`
...
## 16) 2026-03-05 Hardening Update (Critical)

Security + stability hardening ပြုလုပ်ထားသည့် အချက်အလက်အပြည့်အစုံကို ဒီ file မှာ ကြည့်နိုင်ပါတယ်။

- `docs/SECURITY_STABILITY_AUDIT_MM_2026-03-05.md`
