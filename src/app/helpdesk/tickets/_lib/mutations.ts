...
import { syncTicketOutbound } from "@/libs/ticket-outbound-sync";
import { HELPDESK_CACHE_PREFIXES } from "../../cache/redis-keys";
import { writeSystemLog } from "@/libs/logger"; // 🌟 Logger ကိုသာ Import လုပ်ထားပါသည်
...
export async function createTicket(
  ...
  try {
    ...
    // 🌟 Ticket Create Success Log
    writeSystemLog("action", `[TICKET_CREATED] Ticket ID: ${ticket.id} created successfully by User ID: ${userId}`);
    ...
    // 🌟 OTRS API Success Log
    writeSystemLog("external-api", `[OTRS_SYNC_SUCCESS] Real-time sync successful for created Ticket ID: ${ticket.id}`);
    // ==========================================
    ...
    // 🌟 Ticket Create Error Log
    writeSystemLog("error-logs", `[TICKET_CREATE_ERROR] Failed to create ticket: ${errorMsg}`);
    return { error: toErrorMessage(error, "Failed to create ticket.") };
  }
}
...
export async function updateTicket(
  ...
  try {
    ...
    // 🌟 Ticket Update Success Log
    writeSystemLog("action", `[TICKET_UPDATED] Ticket ID: ${updated.id} updated successfully by User ID: ${currentUserId}`);
    ...
    await handleOtrsTicketUpdate(
      ...
      normalizedRemark,
      newAudit,
      imageSync.data.urlsToDelete
    );

    // 🌟 OTRS API Success Log
    writeSystemLog("external-api", `[OTRS_SYNC_SUCCESS] Real-time sync successful for updated Ticket ID: ${updated.id}`);
    // ==========================================

    return { data: { updated, urlsToDelete: imageSync.data.urlsToDelete } };
  } catch (error) {
    const errorMsg = error instanceof Error ? error.message : String(error);
    // 🌟 Ticket Update Error Log
    writeSystemLog("error-logs", `[TICKET_UPDATE_ERROR] Failed to update ticket ID ${ticketId}: ${errorMsg}`);
    return { error: toErrorMessage(error, "Failed to update ticket.") };
  }
}
...
export async function deleteTickets(ticketIds: string[]): Promise<TicketActionResult<{ ids: string[] }>> {
  ...
  try {
    ...
    // 🌟 Ticket Delete Success Log
    writeSystemLog("action", `[TICKET_DELETED] Tickets successfully archived by Admin. IDs: ${idsToArchive.join(", ")}`);

    return { data: { ids: idsToArchive } };
  } catch (error) {
    const errorMsg = error instanceof Error ? error.message : String(error);
    // 🌟 Ticket Delete Error Log
    writeSystemLog("error-logs", `[TICKET_DELETE_ERROR] Failed to delete tickets: ${errorMsg}`);
    return { error: toErrorMessage(error, "Failed to delete tickets.") };
  }
}
...
async function triggerPostMutationHooks(params: {
  ...
  if (!syncResult.ok && !syncResult.skipped) {
    ...
    writeSystemLog("error-logs", `[TICKET_SYNC_ERROR] Outbound ${logAction} sync failed for Ticket ID: ${params.ticketId}. Error: ${syncResult.error}`); // 🌟 Outbound Sync Error Log
