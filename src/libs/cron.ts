...
// 🌟 1. Uncaught Exceptions များကို ဖမ်းခြင်း (ဥပမာ - Syntax Errors, Null variable ခေါ်သုံးမိခြင်း)
process.on("uncaughtException", (error: Error) => {
    writeSystemLog("SYSTEM_ERROR", `[SYSTEM_CRASH] Uncaught Exception detected: ${error.message} | Stack: ${error.stack}`);
});

// 🌟 2. Unhandled Rejections များကို ဖမ်းခြင်း (ဥပမာ - Promise တွေမှာ try-catch မပါဘဲ Error တက်သွားခြင်း)
process.on("unhandledRejection", (reason: unknown) => {
    const errorMsg = reason instanceof Error ? `${reason.message} | Stack: ${reason.stack}` : String(reason);
    writeSystemLog("SYSTEM_ERROR", `[SYSTEM_CRASH] Unhandled Promise Rejection detected: ${errorMsg}`);
});
...
// ==========================================
// 🌟 HELPER FUNCTION: UI & Cache Auto-Update
// ==========================================
async function notifyUiOfTicketChange(ticketId: string) {
    try {
        const ticket = await prisma.ticket.findUnique({
        ...
        writeSystemLog("error-logs", `[CRON_ERROR] Failed to notify UI of ticket change: ${error}`);
    }
}
...
cron.schedule("30 2 * * *", async () => {
    try {
        ...
        if (result.count > 0) {
            await invalidateCacheByPrefixes([HELPDESK_CACHE_PREFIXES.alerts]);
            // 🌟 Standard English Log
            writeSystemLog("cron-jobs", `[ZABBIX_CLEANUP] Successfully purged ${result.count} outdated Zabbix alerts (older than 30 days).`);
        }
    } catch (err) {
        writeSystemLog("error-logs", `[CRON_ERROR] Alerts cleanup failed: ${err}`);
    }
}, { timezone: "Asia/Yangon" });
...
cron.schedule("*/1 * * * *", async () => {
    try {
        ...
        if (violatedTickets.length > 0) {
            // 🌟 Standard English Log
            writeSystemLog("cron-jobs", `[SLA_VIOLATION] Detected and updated ${violatedTickets.length} tickets exceeding SLA resolution time.`);

            emitTicketsChanged({ action: "sla-violated", count: violatedTickets.length, at: new Date().toISOString() });
            emitSlaViolationsChanged({ count: violatedTickets.length, at: new Date().toISOString() });
        }
    } catch (err) {
        writeSystemLog("error-logs", `[CRON_ERROR] SLA check failed: ${err}`);
    }
}, { timezone: "Asia/Yangon" });

// ==========================================
// 🌟 SHARED FUNCTION: Process OTRS Job
// ==========================================
interface QueueJob {
    id: string;
    ticketId: string;
...
async function processOtrsJob(job: QueueJob) {
    ...
    if (job.operation === "TicketCreate") {
        ...
        if (otrsRes.status >= 200 && otrsRes.status < 300 && otrsRes.data.TicketNumber) {
            ...
            // 🌟 Standard English Log
            writeSystemLog("external-api", `[OTRS_CREATE_SUCCESS] Ticket successfully synchronized and created in OTRS. (Local ID: ${job.ticketId}, OTRS Number: ${otrsTicketNumStr})`);
        ...
        // 🌟 Standard English Log
        writeSystemLog("external-api", `[OTRS_UPDATE_SUCCESS] Ticket successfully updated in OTRS. (Local ID: ${job.ticketId})`);
    }
...
// ==========================================
// 🚀 4. OTRS Sync Pending Retry Job (1 Min)
// ==========================================
let isOtrsPendingSyncRunning = false;

cron.schedule("*/1 * * * *", async () => {
    if (isOtrsPendingSyncRunning) {
        return; // Skip cycle silently to avoid log spam
    }
    ...
    try {
        ...
        for (const job of pendingJobs) {
            try {
                ...
                // 🌟 Standard English Log
                writeSystemLog("external-api", `[OTRS_SYNC_ERROR] Synchronization failed for Ticket ID: ${job.ticketId} (Operation: ${job.operation}). Reason: ${errorMsg}`);
                ...
                if (newRetryCount >= 5) {
                    ...
                    // 🌟 Standard English Log
                    writeSystemLog("cron-jobs", `[OTRS_SYNC_PERMANENT_FAIL] Max retries reached (5/5). Permanent synchronization failure for Ticket ID: ${job.ticketId}.`);
                    ...
                    if (systemCommenterId) {
                        try {
                            ...
                            writeSystemLog("error-logs", `[CRON_ERROR] Failed to write failure comment: ${commentErr}`);
                        }
                    }
        ...
        writeSystemLog("error-logs", `[CRON_ERROR] OTRS Sync check failed: ${err}`);
    } finally {
        isOtrsPendingSyncRunning = false;
    }
}, { timezone: "Asia/Yangon" });
...
// ==========================================
// 🚀 5. OTRS Sync Failed Retry Job (10 Min)
// ==========================================
let isOtrsFailedSyncRunning = false;
...
cron.schedule("*/10 * * * *", async () => {
    ...
    try {
        ...
        for (const job of failedJobs) {
            try {
                // 🌟 Standard English Log
                writeSystemLog("cron-jobs", `[OTRS_RETRY_INITIATED] Initiating delayed retry (10-min interval) for previously failed Ticket ID: ${job.ticketId}.`);
                ...
                // 🌟 Standard English Log
                writeSystemLog("external-api", `[OTRS_RETRY_FAILED] Delayed retry attempt failed for Ticket ID: ${job.ticketId}. Reason: ${errorMsg}`);
        ...
        writeSystemLog("error-logs", `[CRON_ERROR] 10-Min OTRS Sync check failed: ${err}`);
    } finally {
        isOtrsFailedSyncRunning = false;
    }
}, { timezone: "Asia/Yangon" });