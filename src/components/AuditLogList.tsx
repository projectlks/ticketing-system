...
// တကယ့် project တွင် "@/libs/myanmar-date-time" အား မှန်ကန်စွာ import လုပ်ပါ
const formatMyanmarDateTime = (date: Date | string | number) => {
  ...
  return (
    <ul
      ...
      {items.map((item, idx) => {
        ...
        const actorName =
          item.user?.name ?? (isAutomated ? "Automation" : "Unknown");
        const actorEmail = item.user?.email ?? (automationSource || "-");
        const createdText = isAutomated
          ? `Ticket created (Automated${automationSource ? `: ${automationSource}` : ""})`
          : "Ticket created";
        ...
        if (item.syncState) {
          if (typeof item.syncState === "string") {
            try {
              parsedSyncState = JSON.parse(item.syncState) as Record<
                string,
                unknown
              >;
            } catch {
            ...
            typeof item.syncState === "object" &&
            item.syncState !== null
          ) {
            parsedSyncState = item.syncState as Record<string, unknown>;
          }
        }
        ...
        return (
          <li key={key} className="relative pl-8 group ">
            ...
            <article
              ...
              {/* NATIVE ACCORDION: Hidden by default, click to expand */}
              {isOtrsSyncFailed && (
                <details className="mt-3 group/error">
                  ...
                  <div className="mt-2 flex items-start gap-2 rounded-md border border-red-100 bg-red-50 p-2.5 text-xs text-red-600">
                    <p>
                      ...
                      Failed to sync this change to OTRS. (The system is
