...
export function AuditLogList({
  ...
  return (
    ...
    >
      ...
      <div
        aria-hidden="true"
        className="absolute left-[14px] top-0 h-full border-l border-border"
      />
      ...
      {items.map((item, idx) => {
        ...
        return (
          <li key={key} className="relative pl-8">
            <span
              aria-hidden="true"
              className="absolute left-1 top-2 inline-flex h-3.5 w-3.5 items-center justify-center rounded-full bg-foreground ring-4 ring-background"
            />
            <div className="rounded-lg border bg-card/60 p-3 shadow-indigo-500 transition-colors hover:bg-accent/40">
              ...
              <p className="mt-2 text-sm leading-relaxed break-words">
                Changed <b className='text-indigo-500'> {item.field}</b> from <i className='text-indigo-500'>&quot;{item.oldValue}&quot;</i> to <i className='text-indigo-500'>&quot;{item.newValue}&quot;</i>
              </p>
